...
const ActionMenu = ({ setActive, unlockTier, included, temp }) => {
    return <div className="actionMenu" style={{ midWidth: "400px", height: '100%' }}>
        <div style={{}}>
            ...
            <Action included={included["spin"] && unlockTier > 2} locked={unlockTier < 1.5} text={"Centrifuge"} onClick={() => setActive("spin")} />
            <Action included={included["buffer"]} locked={unlockTier < 1.7} text={"Buffer"} onClick={() => setActive("buffer")} />
            <Action included={included["polymerase"]} locked={unlockTier < 1.7} text={"Polymerase"} onClick={() => setActive("polymerase")} />
            <Action included={included["dntp"]} locked={unlockTier < 1.7} text={"dNTP"} onClick={() => setActive("dntp")} />
            <Action included={included["primer"]} locked={unlockTier < 1.7} text={"Primer"} onClick={() => setActive("primer")} />
